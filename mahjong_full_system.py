...
HU_CARDS_DICT = {
    ...
    "30 番 惊世豪强牌型": {
        "小四喜": 30, "小三元": 30, "孔雀东南飞（小）": 30, "西北下大雪（小）": 30, "大七对": 30,"混老头": 30
    },
    "20 番 纵横捭阖牌型": {
        "清一色": 20, "清带幺": 20, "一色四步高": 20, "一色四同顺": 20, "一色四节高": 20, 
        "三杠子": 20, "四连刻": 20, "七小对": 20, "全将碰": 20
    },
    "10 番 名震四海牌型": {
    ...
    "2 番 积少成多牌型": {
        "自摸": 2, "断幺九": 2, "幺九刻": 2, "暗杠": 2, "独听边张（卡窿）": 2
    },
    "1 番 风起青萍牌型": {
...
def save_github_db():
    ...
    try:
        ...
        st.error(f"☁️ 云端同步发生故障（数据已安全保存在本地）：{str(e)}")
        return False
...
div[data-testid="stCheckbox"] { background: rgba(255, 255, 255, 0.65) !important; border: 1px solid rgba(46, 37, 32, 0.08) !important; border-radius: 12px !important; padding: 10px 16px !important; margin-bottom: 8px !important; box-shadow: 0 2px 6px rgba(0,0,0,0.02) !important; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); }
div[data-testid="stCheckbox"]:hover { background: rgba(255, 255, 255, 0.95) !important; border-color: #b91c1c !important; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(185, 28, 28, 0.08) !important; }
div[data-testid="stCheckbox"] label p { font-weight: 600 !important; color: #451a03 !important; }
@media (max-width: 768px) { .big-title { font-size: 1.9rem !important; } .stButton>button { padding: 14px 20px !important; } div[data-testid="stCheckbox"] { padding: 12px 10px !important; } }
</style>
    ...
    with st.sidebar:
        ...
        with col1:
            if st.button("✅ 提交本局", type="primary", use_container_width=True):
                ...
                # 写回本地并强行单局覆盖至云端
                df_updated = pd.DataFrame(st.session_state.current_match)
                with pd.ExcelWriter(DB_FILE) as writer:
                    for s_name, s_df in db.items():
        ...
        if st.button("☁️ 将整场封盘保存至云端", use_container_width=True):
            if len(st.session_state.current_match) == 0:
                ...
                with st.spinner("正在安全同步至本地并覆盖云端..."):
                    old_summary = db["比赛汇总"]
                    df = pd.DataFrame(st.session_state.current_match)
                    ...
                    summary_row = pd.DataFrame([{
                        ...
                        "第1名": rank_result[0][0], "第1名名次分": 5, "第2名": rank_result[1][0], "第2名名次分": 3,
                        "第3名": rank_result[2][0], "第3名名次分": 2, "第4名": rank_result[3][0], "第4名名次分": 1
                    }])
                    ...
                    if save_github_db():
                        st.cache_data.clear()
                        st.toast("🎉 本地数据保存成功，并已强制完全同步至云端！", icon="✅")
...
# ---------------------- 📜 历史比赛查询 ----------------------
    ...
    st.header("📜 历史对局封存卷轴")
    with st.spinner("正在开启数据古籍..."): db = load_github_db()
    summary_df = db["比赛汇总"]
    if len(summary_df) == 0:
        st.info("未发现历史封存记录")
    else:
